...
import { OpenAIIcon } from "./_icons/openAI";
import Link from "next/link";
import { NeonIcon } from "./_icons/neonIcon";
import { ClerkIcon } from "./_icons/cleckIcon";
...
export default function Home() {
  return (<>
    ...
    <section className="bg-primary text-primary-foreground">
      ...
      <h2 className="text-center text-3xl text-balance">
    ...
    <section id="pricing" className="px-8 py-16 bg-accent/5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-screen-xl mx-auto">
          {subscriptionTiersInOrder.map(tier=> (<PricingCard key={tier.name} {...tier}/>))}
      </div>
    </section>
    <footer className="px-10 pt-16 pb-8 flex flex-col sm:flex-row gap-8 sm:gap-4 justify-between items-start ">
      <Link href={"/"}>
        <BrandLogo/>
      </Link>
      <div className="sm:flex gap-8 md:justify-evenly md:w-full">
        ...
        <div className="flex flex-col gap-8">
          <FooterLinkGroup
              title="Features"
              links={[{ label: "Dealfit Discounts", href: "#" }]}
            />
            <FooterLinkGroup
              title="Tools"
              links={[
...
function PricingCard ({name, priceInCents,  maxNumberOfProducts, maxNumberOfVisits, canRemoveBranding, canAccessAnalytics, canCustomizeBanner,}:(typeof subscriptionTiersInOrder)[number]){
 const isMostPopular = name === "Standard";
 return <Card>
  <CardHeader>
    <div className="text-accent font-semibold mb-8 ">{name}</div>
    <CardTitle className="texl-xl font-bold">${priceInCents / 100} /mo</CardTitle>
    <CardDescription>
  ...
  <CardFooter className="flex flex-col gap-4 items-start">
      <Feature className="font-bold">{maxNumberOfProducts} product{maxNumberOfProducts>1?"s":""}</Feature>
